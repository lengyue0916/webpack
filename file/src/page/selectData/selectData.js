...
Page(Object.assign({
  ...
  onLoad: function (options) {
    ...
    var checkInDate = options.checkInDate?options.checkInDate:Moment(new Date()).add(8,'day').format('YYYY-MM-DD');
    var checkOutDate = options.checkOutDate?options.checkOutDate:Moment(new Date()).add(9,'day').format('YYYY-MM-DD');
    
    // var year = new Date(checkInDate).getFullYear();
    // var month = new Date(checkInDate).getMonth()+1;
    // var day = new Date(checkInDate).getDate();

    // var year_out = new Date(checkOutDate).getFullYear();
    // var month_out = new Date(checkOutDate).getMonth()+1;
    // var day_out = new Date(checkOutDate).getDate();
...
　　　　//当前选择的日期为同一个月并小于今天，或者点击了空白处（即day<0），不执行
    ...
    if((this.data.markcheckInDate && Moment(dates).before(this.data.checkInDate)||this.data.checkInDate === dates)){
